...
路径约定：
- `references/...` 相对当前 skill 目录（`${SKILL_ROOT}/references/...`）。

默认加载清单：
- L1（启动前）：`references/genre-tropes.md`
- L2（按需）：
  - 题材模板：`references/genres/{genre}.md`
  - 金手指：`references/golden-finger-templates.md`
...
- `Read/Grep`：读取项目上下文与参考文件（`README.md`、`CLAUDE.md`、`references/genres/*`、`references/*`）。
...
export SKILL_ROOT="${WORKSPACE_ROOT}/.claude/skills/init-project"
```
...
- 加载最小参考：
  ...
  - `references/genre-tropes.md`
  - `references/genres/`（仅在用户选定题材后按需读取）
